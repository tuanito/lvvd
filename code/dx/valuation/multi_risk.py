...
class valuation_mcs_european_multi(valuation_class_multi):
    ...
    def generate_payoff(self, fixed_seed=True):
        ...
        matches = np.where(time_grid == self.maturity)[0]
        if len(matches) == 0:
            raise ValueError(
                'Maturity date %r not in time grid of underlying.'
                % self.maturity)
...
class valuation_mcs_american_multi(valuation_class_multi):
    ...
    def generate_payoff(self, fixed_seed=True):
        ...
        self.instrument_values = {key: name.instrument_values for key, name
                                  in self.underlying_objects.items()}
        start_matches = np.where(self.time_grid == self.pricing_date)[0]
        end_matches = np.where(self.time_grid == self.maturity)[0]
        if len(start_matches) == 0 or len(end_matches) == 0:
            raise ValueError(
                'Pricing date %r or maturity date %r not in time grid '
                'of underlying.' % (self.pricing_date, self.maturity))
        time_index_start = int(start_matches[0])
        time_index_end = int(end_matches[0])
        instrument_values = {}
